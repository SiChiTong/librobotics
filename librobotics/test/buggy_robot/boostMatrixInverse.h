#ifndef BOOSTMATRIXINVERSE_
#define BOOSTMATRIXINVERSE_
...
boost::numeric::ublas::matrix<T> gjinverse(
		const boost::numeric::ublas::matrix<T> &m, bool &singular) {
		...
		for (int k = 0; k < size; k++) {
			if (A(k, k) == 0) // XXX: test for "small" instead
			{
				...
				if (l < 0) {
					...
					std::cerr << __FUNCTION__ << ":" << "Swapped row " << k
							<< " with row " << l << ":" << A << "\n";
#endif
				}
			}
...
//#if defined(DEBUG) || !defined(NDEBUG)
//		std::cerr << __FUNCTION__ << ":" << "GJ row " << kk << " : " << A
//				<< "\n";
//#endif
	}
